...
Detailed functional and non-functional requirements for the Smart Algo Trading system (Indian market, Dhan API, AI/LLM).

---

## 1. Functional Requirements

### 1.1 Flow 1 — Portfolio Mode

#### 1.1.1 New Portfolio

| ID   | Requirement | Priority |
|------|-------------|----------|
| R1.1 | User shall be able to enter total portfolio capital in Indian Rupees (e.g. 1 Lakh, 5 Lakh). Accepted formats: numeric (100000), "1L", "1 Lakh". | Must |
...
| ID   | Requirement | Priority |
|------|-------------|----------|
| R1.8 | User shall be able to upload a portfolio file (CSV or Excel) with columns: at least symbol, quantity; optionally avg cost, current price, or value. | Must |
...
| R1.12 | User shall be able to request rebalancing using traditional methods. Inputs: current holdings (from upload or session), target allocation (e.g. 60% equity / 40% debt, or sector weights), and rebalancing rule (bands e.g. ±5%, or calendar). | Must |
| R1.13 | System shall return rebalancing output: current weight per holding/segment, target weight, difference; suggested buy/sell list (symbol, action, quantity or amount) to bring portfolio toward target. | Must |
| R1.14 | No algo-run (no Dhan market data or LLM) is required for the existing-portfolio path; analysis and rebalancing only. | Must |
...
| R4.2 | Option selling algo: inputs from Dhan options chain (strike, IV, OI); output Sell CE/PE, Hold, Close; position size from margin/premium risk. | Must |
...
| ID   | Requirement | Priority |
|------|-------------|----------|
| R5.1 | System shall integrate with Dhan API: authentication (access token + client_id), market data (LTP, OHLC, options chain for F&O), and optionally orders (place_order). | Must |
...
| R5.4 | Rate limits: respect Dhan API limits (e.g. 5/sec data, 10/sec orders); throttle or queue requests as needed. | Must |
...
| R6.3 | Order placement via Dhan: security_id, exchange_segment, transaction_type, quantity, order_type, product_type. | Must |
...
| ID    | Requirement | Priority |
|-------|-------------|----------|
| NFR1  | Backend shall be implemented in Python 3.10+ using FastAPI; frontend in React. Backend shall use AGNO for agents and Pydantic for request/response and agent JSON output schemas. | Must |
| NFR2  | API responses shall be JSON; appropriate HTTP status codes and error payloads (errorCode, errorMessage). | Must |
| NFR3  | Sensitive data (API keys, Dhan tokens) shall be stored via environment variables or secure config; not committed to repo. | Must |
...
- Support for brokers other than Dhan (future extension possible).
