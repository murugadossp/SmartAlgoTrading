...
| T0.5    | Add **config module** (e.g. `app/config/`): global settings (Pydantic from env); agent defaults in `config/config.yaml` under **agents.default**. Keep separate from agent code. | -     | T0.2       | See design §2.2.1 and [config_agents.md](config_agents.md). |
